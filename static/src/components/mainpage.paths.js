...
/**
 * Generates the HTML structure for a new Paths query (All/Shortest) tab header.
 *
 ...
 * @param {string} typePath The type of paths to be found.
 * @return {!jQuery} A jQuery object representing the tab header element.
 */
function pathsTab_UI_TabHeader(index, typePath) {
    // Define the type of paths to be found
    const pathTitle = String(typePath).toLowerCase() == "allpaths" ? "All Paths" : "Shortest Path";
    ...
    let tabHeader = $("<li>", {
        ...
        $("<button>", {
            ...
            $("<span>", {"class": "ms-1"}).append(document.createTextNode(`${pathTitle} (${index})`)),
...
/**
 * Generates the HTML structure for a new Paths query (All/Shortest) tab content.
 *
 * This function constructs a Bootstrap-based UI panel containing 
 * input fields for graph patterns (paths), a filter fromExpr with column 
 * suggestion tools derived from the current data context, a filter toExpr with 
 * column suggestion tools derived from the current data context, a filter 
 ...
 */
function pathsTab_UI_TabContent(context, index, job_id, typePath) {
    ...
    let tabContent = $("<div>", {
        ...
        $("<div>", {"class":"p-3","style": "display: table; table-layout: fixed; width: 100%;"})
            .append(
                        ...
                        $("<div>", {"class":"col m-0 p-0 d-flex"})
                            .append(
                                ...
                                $("<div>", {"class":"input-group mb-3 border border-dark-subtle rounded"})
                                    .append(
                                        $("<input>", {"type":"text","class":"form-control font-monospace paths-fromExpr-input","maxlength":"2048","aria-label":"From Vertex","style":"font-size:0.80rem;","data-query-index": index}),
                ...
                // --- To Vertex Expression Field Row ---
                $("<div>", {"class":"row g-3 mb-3 align-items-center"})
                    .append(
                        $("<div>", {"class":"col m-0 p-0","style":"max-width: 150px;"})
                            .append(
                                $("<label>", {"class":"form-label pb-3"}).append(document.createTextNode("To Vertex"))),
                        $("<div>", {"class":"col m-0 p-0 d-flex"})
                            .append(
                                // Dropdown tool to inject Vertex column names into the filter.
                                $("<div>", {"class":"input-group mb-3 border border-dark-subtle rounded"})
                                    .append(
                                        $("<input>", {"type":"text","class":"form-control font-monospace paths-toExpr-input","maxlength":"2048","aria-label":"To Vertex","style":"font-size:0.80rem;","data-query-index": index}),
                                        $("<button>", {"class":"btn btn-outline-secondary dropdown-toggle border-0","type":"button","data-bs-toggle":"dropdown","aria-expanded":"false","data-query-index": index})
                                            .append($("<i>", {"class":"fa-solid fa-circle-user"})),
                                        $("<ul>", {"class":"dropdown-menu dropdown-menu-end scrollable-list query-column-tool","data-query-index": index})
                                            .append((function() {
                    ...
                    .append(
                        ...
                        $("<div>", {"class":"col m-0 p-0 d-flex"})
                            .append(
                                $("<div>", {"class":"input-group mb-3 border border-dark-subtle rounded"})
                                    .append(
                                        $("<input>", {"type":"text","class":"form-control font-monospace paths-edgeExpr-input","maxlength":"2048","aria-label":"Edges Filter","style":"font-size:0.80rem;","data-query-index": index}),
                                        $("<button>", {"class":"btn btn-outline-secondary dropdown-toggle border-0","type":"button","data-bs-toggle":"dropdown","aria-expanded":"false","data-query-index": index})
                    ...
                    .append(
                        ...
                        $("<div>", {"class":"col m-0 p-0 d-flex"})
                            .append(
                                $("<input>", {
                                    "type":"number",
                                    "class":"px-2 py-2 border border-dark-subtle rounded paths-max-distance-input",
                                    "placeholder":"3",
                                    "step":"1",
                                    "min":"1",
                                    "max":"10",
...
/**
 * Orchestrates the creation and event binding of a new Paths query (All/Shortest) tab.
 *
 ...
 */
function pathsTab(context, typePath) {
    // --- Pre-validation ---
    ...
    // --- UI Injection ---
    const tabHeader = pathsTab_UI_TabHeader(index, typePath);
    $(".graphTabHeader").append(tabHeader);

    const job_id = `job_find_${index}_${Date.now()}`;
    const tabContent = pathsTab_UI_TabContent(context, index, job_id, typePath);
    ...
    $(`.btn-execute-command[data-query-index=${index}]`)
        ...
        .on("click", function() {
            ...
            const api_params = {
                ...
                fromExpr: $(`.paths-fromExpr-input[data-query-index=${idx}]`).val(),
                toExpr: $(`.paths-toExpr-input[data-query-index=${idx}]`).val(),
                max_distance: parseInt($(`.paths-max-distance-input[data-query-index=${idx}]`).val()),
                edgeExpr: $(`.paths-edgeExpr-input[data-query-index=${idx}]`).val() || undefined,
            ...
            $.ajax({
                type: "GET",
                url: String(typePath).toLowerCase() == "allpaths" ? "./api/graph/allpaths" : "./api/graph/shortestpath",
...
/**
 * @fileoverview Export definitions for the Paths (All / Shortest) Management module.
